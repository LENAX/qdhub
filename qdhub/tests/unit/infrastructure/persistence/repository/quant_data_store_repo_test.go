...
func TestTableSchemaRepository_Create(t *testing.T) {
	db, cleanup := setupQuantDataStoreTestDB(t)
	defer cleanup()
	...
	if schema.ID.IsEmpty() {
		t.Error("TableSchema ID should be set after creation")
	}
}

func TestTableSchemaRepository_Get(t *testing.T) {
	db, cleanup := setupQuantDataStoreTestDB(t)
	defer cleanup()
	...
	got, err := schemaRepo.Get(schema.ID)
	if err != nil {
		t.Fatalf("Get() error = %v", err)
	}
	...
	if got.ID != schema.ID {
		t.Errorf("Get() ID = %s, want %s", got.ID, schema.ID)
	}
}

func TestTableSchemaRepository_GetByDataStore(t *testing.T) {
	db, cleanup := setupQuantDataStoreTestDB(t)
	defer cleanup()
	...
	schemas, err := schemaRepo.GetByDataStore(ds.ID)
	if err != nil {
...
func TestTableSchemaRepository_Update(t *testing.T) {
	db, cleanup := setupQuantDataStoreTestDB(t)
	defer cleanup()
...
func TestTableSchemaRepository_Delete(t *testing.T) {
	db, cleanup := setupQuantDataStoreTestDB(t)
	defer cleanup()
	...
	err = schemaRepo.Delete(schema.ID)
	if err != nil {
		t.Fatalf("Delete() error = %v", err)
	}
	...
	if got != nil {
		t.Error("Delete() should remove the schema")
	}
}
