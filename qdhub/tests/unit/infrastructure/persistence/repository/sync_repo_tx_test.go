...
func setupSyncRepoTestDB(t *testing.T) (*persistence.DB, func()) {
	...
	_, err = db.Exec(`
		CREATE TABLE IF NOT EXISTS sync_plan (
			...
			plan_mode VARCHAR(32) NOT NULL DEFAULT 'batch',
			selected_apis TEXT NOT NULL,
			resolved_apis TEXT,
			execution_graph TEXT,
			cron_expression VARCHAR(128),
			schedule_start_cron VARCHAR(128),
			schedule_end_cron VARCHAR(128),
			schedule_pause_start_cron VARCHAR(128),
			schedule_pause_end_cron VARCHAR(128),
			pull_interval_seconds INTEGER DEFAULT 0,
