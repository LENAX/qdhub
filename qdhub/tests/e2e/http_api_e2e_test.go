...
func setupE2EHTTPServer(t *testing.T, db *persistence.DB, taskEngine *engine.Engine) (*httptest.Server, func()) {
	...
	syncSvc := impl.NewSyncApplicationService(syncPlanRepo, cronCalculator, planScheduler, dataSourceRepo, dataStoreRepo, workflowExecutor, dependencyResolver, taskEngineAdapter, uowImpl, metadataRepo, nil, nil, "", nil)
	...
	server := httpapi.NewServer(config, authSvc, metadataSvc, dataStoreSvc, nil, syncSvc, workflowSvc, nil, nil, nil, nil, nil, nil, jwtManager, enforcer, "")
