...
2. **前端无感**：前端始终连接同一 WebSocket 读 `LatestQuoteStore`，不关心当前是哪个源在写。
3. **生产故障切换**：生产环境**两个 data collector 同时启动**；只从**当前选中的** collector 读取数据写 Store。当检测到**当前选中的** collector 故障（网络中断、被 ban 等）时，**切换到另一个** collector（另一路已在跑），立即从新源写 Store，前端无感。
...
- **RealtimeSourceSelector**：新组件，维护 `active`（`tushare_ws` | `sina`），默认 `tushare_ws`。暴露 `ShouldWriteToStore(source string) bool`、`SwitchTo(source string)`。
- **WS 写 Store**（[ws_collector.go](internal/infrastructure/datasource/tushare/realtime/ws_collector.go) 或下游 job）：写前调用 `Selector.ShouldWriteToStore("tushare_ws")`，仅 true 时写 Store。
- **Sina 写 Store**（[realtime_jobs.go](internal/infrastructure/taskengine/jobs/realtime_jobs.go)）：`RealtimeQuoteStreamHandlerJob` 写 DuckDB 后，若 `ShouldWriteToStore("sina")` 则对每行写 Store。开发环境仅 Sina 时可直接写或 Selector 恒为 sina。
- **两表字段差异**：见 [sina_realtime_quote.sql](doc/example/sina_realtime_quote.sql) 与 [ts_realtime.sql](doc/example/ts_realtime.sql)。写 Store 时若前端依赖 Tushare 字段名，需对 realtime_quote 行做字段映射。
...
**故障切换触发条件**：在将某次执行标记为失败时，若为**生产**且计划为**实时**，且**该失败执行正好对应当前 RealtimeSourceSelector 的 active**（active 为 `tushare_ws` 且失败 exec 的 `SyncedAPIs` 含 `ts_realtime_mkt_tick`；或 active 为 `sina` 且含 `realtime_quote`），则调用 `RealtimeSourceSelector.SwitchTo(另一源)`，**不**将计划置为已完成。另一源 workflow 已在运行，切换后其数据立即写入 Store。

**失败检测发生处**：GetExecutionProgress、HandleExecutionCallback、ReconcileRunningExecutions。在 exec 被标为 Failed 并持久化后，若为生产 + 实时且该 exec 对应当前 active，则 Selector.SwitchTo(另一源)，不执行 plan.MarkCompleted。
...
| **RealtimeSourceSelector**（新） | active（tushare_ws \| sina）；ShouldWriteToStore(source)；SwitchTo(source)；默认 active=tushare_ws |
...
| sync_impl.go | ExecuteSyncPlan：production 且 ts_realtime_mkt_tick 时两次调用执行层（WS + Sina），创建两个 execution；GetExecutionProgress / HandleExecutionCallback / ReconcileRunningExecutions：失败 exec 对应当前 active 时 Selector.SwitchTo(另一源)，不 MarkCompleted(plan) |
| frontend-realtime-integration.md | 补充 QDHUB_ENV、生产双 Collector 与切换、开发仅 Sina |
