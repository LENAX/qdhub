...
func MetadataRefreshStartHandler(tc *task.TaskContext) {
	logrus.Printf("[MetadataRefresh] 🚀 Workflow started - Task: %s, TaskID: %s, WorkflowInstanceID: %s",
...
func MetadataRefreshSuccessHandler(tc *task.TaskContext) {
	logrus.Printf("[MetadataRefresh] ✅ Task succeeded - Task: %s, TaskID: %s",
	...
	if result != nil {
		if resultMap, ok := result.(map[string]interface{}); ok {
			...
			if savedCount, ok := resultMap["saved_count"]; ok {
				logrus.Printf("[MetadataRefresh] 💾 Saved %v items", savedCount)
			}
			if apiCount, ok := resultMap["api_count"]; ok {
				logrus.Printf("[MetadataRefresh] 📡 Found %v APIs", apiCount)
			}
			// 模板任务生成的子任务数量
			if generated, ok := resultMap["generated"]; ok {
				logrus.Printf("[MetadataRefresh] 🔄 Generated %v sub-tasks", generated)
			}
		}
	}
}
...
func MetadataRefreshFailureHandler(tc *task.TaskContext) {
	...
	logrus.Printf("[MetadataRefresh] ❌ Task failed - Task: %s, TaskID: %s, Error: %s",
...
func MetadataRefreshCompleteHandler(tc *task.TaskContext) {
	...
	logrus.Printf("[MetadataRefresh] 🏁 Workflow completed - WorkflowInstanceID: %s, Status: %s",
...
func LogProgressHandler(tc *task.TaskContext) {
	logrus.Printf("[Progress] 📊 Task: %s, TaskID: %s, WorkflowInstanceID: %s",
		tc.TaskName, tc.TaskID, tc.WorkflowInstanceID)
}
	...
	if result != nil {
		if resultMap, ok := result.(map[string]interface{}); ok {
			if status, ok := resultMap["status"]; ok {
				logrus.Printf("[Success] Status: %v", status)
			}
		}
	}
}
